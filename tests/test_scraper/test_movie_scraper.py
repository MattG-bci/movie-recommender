...
class TestMovieScraper:
    @pytest.mark.asyncio
    async def test_scrape_movies(self, fake_site_url):
    ...
    @pytest.mark.asyncio
    async def test_movie_metadata(self, fake_site_url):
        scraper = MovieScraper(movie_page_url=f"{fake_site_url}/films/popular/")
    ...
    @pytest.mark.asyncio
    async def test_skips_existing_movies(self, fake_site_url):
        scraper = MovieScraper(movie_page_url=f"{fake_site_url}/films/popular/")

        movies = await scraper.get_data_incremental(
            existing_movie_titles=["The Matrix"]
        )
